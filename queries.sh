...
echo "$($PSQL "select distinct(teams.name) from teams inner join games on teams.team_id=games.winner_id where games.year=2014 and games.round='Eighth-Final' union select distinct(teams.name) from teams inner join games on teams.team_id=games.opponent_id where games.year=2014 and games.round='Eighth-Final'")"
